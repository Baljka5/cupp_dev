...
class VeritechOnlyAPIKeyAuthentication(BaseAuthentication):
    def authenticate(self, request):
        api_key = request.META.get('HTTP_X_API_KEY') or request.GET.get('api_key')
        expected_key = getattr(settings, 'HR_API_KEY', None)

        if not api_key or api_key != expected_key:
            raise AuthenticationFailed('Invalid or missing API key for Veritech endpoint')
